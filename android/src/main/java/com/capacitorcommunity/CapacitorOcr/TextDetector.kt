...
public class TextDetector {
    public fun detectText(call: PluginCall, bitmap: Bitmap) {
                    ...
                    for (block in detectedBlocks.textBlocks) {
                        for (line in block.lines) {
                            ...
                            val cornerPoints =
                                line.cornerPoints
                                    ?: throw NoSuchPropertyException(
                                        "FirebaseVisionTextRecognizer.processImage: could not get bounding coordinates"
                                    )
                            ...
                            val textDetection = mapOf(
                                ...
                                "bottomLeft" to
                                    listOf<Double?>((bottomLeft.x).toDouble() / width, (height - bottomLeft.y).toDouble() / height),
                                "bottomRight" to
                                    listOf<Double?>((bottomRight.x).toDouble() / width, (height - bottomRight.y).toDouble() / height),
                                "text" to line.text
                            )
